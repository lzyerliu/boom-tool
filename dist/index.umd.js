(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.BoomTool = {}));
    ...
    // is object
    var isObject$1 = function (val) { return val !== null && is(val, 'Object'); };
    // is array
    var isArray$1 = function (val) { return Boolean(val) && Array.isArray(val); };
    // is empty
    ...
    var isPromise = function (val) {
        return is(val, 'Promise') && isObject$1(val) && isFunction$1(val.then) && isFunction$1(val.catch);
    };
    // is date
    ...
    ***************************************************************************** */
    ...
    var decimalCompute = function (type) {
        ...
        var bigNumbers = args.map(function (num) { return Math.round(num * power); });
        var result = 0;
        switch (type) {
            case '+':
                result = bigNumbers.reduce(function (preNum, nextNum) { return preNum + nextNum; }, result) / power;
                break;
            case '-':
                result = bigNumbers.reduce(function (preNum, nextNum) { return preNum - nextNum; }) / power;
                break;
            case '*':
                var times = Math.pow(power, bigNumbers.length);
                result = bigNumbers.reduce(function (preNum, nextNum) { return preNum * nextNum; }) / times;
                break;
            case '/':
                result = bigNumbers.reduce(function (preNum, nextNum) { return preNum / nextNum; });
                break;
        }
        return {
            result: result,
            next: function (nextType) {
                ...
                return decimalCompute.apply(void 0, __spreadArray([nextType, result], nextArgs, false));
            }
        };
    };
    ...
    function flatMap(collection, iteratee) {
      return baseFlatten(map(collection));
    }
    ...
    var searchParams = function (search) {
        ...
        // @ts-ignore
        return flatMap(keys).reduce(function (prev, key) {
    ...
    var concatParams = function (url, params, prefix) {
        ...
        if (keys.length) {
            query = keys.reduce(function (str, k, i) {
                str += "".concat(i ? '&' : '').concat(k, "=").concat(params[k]);
                return str;
            }, prefix);
        }
        return url + query;
